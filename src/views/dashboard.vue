...
<template>
  <section class="mx-5 px-5">
    ...
    <div class="py-5 my-5">
      <span class="ps-2">
        Welcome, To<span
          class="card-text-color ps-2 shadow-lg m-3 p-3 card-radius"
          >NFT BOOK SELLER </span
    ...
    <v-card>
      <div class="pt-4">
        <nav style="display: flex; justify-content: space-between" class="pb-5">
          <h3 class="folder-text ps-5">
            <button class="btn button-radius shadow-lg">Home</button>
          </h3>
          <a
            ><button class="btn button-radius shadow-lg">Read & Earn</button>
          </a>
          <div>
            <v-dialog
              v-model="dialog"
    ...
    <nav
      ...
      class="pt-5 mt-4"
    >
      <span class="card-text-color ps-2 shadow-lg m-3 p-3 card-radius"
        ><h3 class="folder-text">
          Sell, Explore and Read Books at
          <span style="color: green">Bookee</span>
        </h3></span
      >
    </nav>
        ...
        <v-col v-for="doc in documents" :key="doc.time" cols="2" sm="8" md="2">
          ...
          >
            ...
            <div style="display: flex; justify-content: space-between">
              <button  class="p-3 btn bg-white">
                SUMMARY
              </button>
              <button
			  @click="buyItem(doc.bookId)"
                style="background-color: "
                class="p-2 btn card-radius shadow-lg"
              >
                BUY
              </button>
            </div>
...
export default {
  ...
  methods: {
    openDoc(cid) {
      let baseUrlr = "http://0.0.0.0:8081/ipfs/";
...
let tokenCid = await client.put(files)

      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner();
      const contractnft = new ethers.Contract(contractAddressnft, abinft, signer);
      const marketCotract = new ethers.Contract (contractAddress,abi,signer)
      const storedResponse = await (await contractnft.mintNft(tokenCid)).wait()
      const bookId = await contractnft.getTokenCounter()
	  await (await contractnft.setApprovalForAll(marketCotract.address,true)).wait()
	  console.log(`bookId ${contractnft.address}`)
	  const listRes = await marketCotract.listBook(contractnft.address,bookId,price)
